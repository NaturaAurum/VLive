...
namespace VLive.Runtime
{
    public class HandsSolver : MonoBehaviour, IHandsSolver
    {
        ...
        private void FingerSolve(Transform hand, IReadOnlyList<PointData> points, bool rightHand)
        {
            var quaternion12 = hand.localRotation * hand.transform.parent.localRotation;
            ...
            for (var i = 0; i < FingerNames.Length; ++i)
            {
                ...
                for(var j = 0; j < PartNames.Length - 1; ++j)
                {
                    ...
                    var vector3_2 = Vector3.Cross(localCoords[index3 + 1], Vector3.up);
